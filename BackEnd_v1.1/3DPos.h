...
struct Struct3D {
    ...
    // C++20: 默认实现的三路比较运算符 (spaceship operator),使得 Struct3D 对象可以直接使用 ==, !=, <, >, <=, >= 进行比较
...
struct PlayerInputState {
    ...
    bool jumpPressed = false; // 仅在跳跃键被按下的那一帧为true
};
...
namespace GameConstants {
    ...
    constexpr float JUMP_FORCE = 7.5f;      // 玩家跳跃的初始垂直速度
    constexpr float GROUND_LEVEL_Y = 0.0f;  // 地面的 Y 坐标
    constexpr float MAX_FALL_VELOCITY = -25.0f; // 最大下落速度
    constexpr float PLAYER_HEIGHT = 1.0f;   // 玩家高度
    constexpr float PLAYER_WIDTH = 1.0f;    // 玩家宽度
    constexpr float PLAYER_DEPTH = 1.0f;    // 玩家深度
    // 之后可以根据需要添加更多常量
}

// 代表一个轴对齐包围盒的结构体
struct AABB {
    Struct3D min; // 包围盒的最小角坐标
    Struct3D max; // 包围盒的最大角坐标
